...
The Solr loader verifies the `_default` configset's `*_t`, `*_s`, and `*_dt`
dynamic fields listed in `schema_solr.json`. The ES/OpenSearch mapping uses
equivalent dynamic templates from `schema_es.json`: `*_t` becomes `text`,
...
│   └── aux-bin/                     # Optional local Solr helper scripts
├── docs/                            # Additional reference material
│   └── images/                      # Video thumbnails and engine logos
├── generate_test_collection/        # Generates and submits synthetic test data
├── reports/                         # Default Markdown and CSV output directory
│   └── processing-stats.jsonl       # Generated, Git-ignored scan history
├── src/                             # Python source tree
│   └── dq/                          # Main Python package
│       ├── reports/                 # Special reports and shared formatting
│       │   └── Various report directories...
│       └── rules/                   # Base, composite, shared, and custom rules
│           └── Various rule directories...
├── tests/                           # Automated test suite
├── dq.ini.template                  # Example supported configuration
├── LICENSE.txt                      # Apache License 2.0 terms
└── README.md                        # Main user and developer documentation
```

In this version, most documentation is in the main `README.md` file. Custom
rules also go under `src/dq/rules/`; see **Part Number Base Rule Directory and
Code** and **Part Number Composite Rule Directory and Code** below.

### Base Rules vs. Composite Rules

A base rule performs one focused test against a stored field value. It can be
configured as a regex rule or implemented with Python code. Examples include
checking for a missing value, surrounding whitespace, suspicious Unicode, or a
specific email format.

A composite rule combines base rules into the behavior users normally want.
This keeps each individual test small while avoiding duplicated rule chains.
For example, an email composite can reuse the standard missing, empty-string,
whitespace, and Unicode checks before applying the email-format regex. A
composite may include base rules or other composites. DQ recursively flattens
the result into one ordered list and runs each base rule at most once.

Before invoking any base rule's regex or Python value logic, DQ checks for a
null value so the rule cannot fail by referencing `None`. A null is a reportable
rule failure by default. Use `--skip_null_values true` when null records should
be omitted instead.

### Directory Layout for Custom Rules

Rules are Python packages under `src/dq/rules/`. A public rule gets one
directory whose name is also its command-line rule name. Directory names ending
in `_base` define base rules; names ending in `_composite` define predefined
composite rules. Supporting files and shared directories do not use either
suffix. See `src/dq/rules/README.md` for the complete package map and built-in
rule catalog.
...
└── src/
    └── dq/
        └── rules/                              # All built-in and custom rules
            ├── code_points_base/               # Procedural base-rule example
            ├── email_base/                     # Regex base rule
            ├── email_composite/                # Standard checks plus email_base
            ├── part_number_example_base/       # Part-number regex base rule
            │   ├── __init__.py                 # Makes this a Python package
            │   ├── rule.ini                    # Base-rule configuration
            │   ├── part_number_example.regex   # Extended regular expression
            │   └── optional_demo_values.py     # Optional dq_demo value generator
            ├── part_number_example_composite/  # Part-number composite rule
            │   ├── __init__.py                 # Makes this a Python package
            │   └── rule.ini                    # Ordered component rules
            ├── text/                           # Shared text-rule support
            ├── regex/                          # Shared regex-rule implementation
            ├── standard_text_composite/        # Reusable standard text checks
            ├── Other rules...
            └── Other supporting files...
```

Every rule directory contains `__init__.py` and `rule.ini`. The INI file must
have a common `[rule]` section and exactly one section for its kind:

| Directory | Required Sections | Purpose |
| --------- | ----------------- | ------- |
| `<RULE_NAME>_base/` | `[rule]` and `[base_rule]` | Tests one stored value and returns a failure reason or passes it. |
| `<RULE_NAME>_composite/` | `[rule]` and `[composite_rule]` | Defines an ordered chain of base rules or nested composite rules. |

The common `[rule]` section contains a `description` for `--list_rules`. It may
also contain `automatic_field_types` and `automatic_field_name_patterns` for
automatic checkup selection. An empty `[base_rule]` section is valid for a
procedural rule. Regex base rules add `report = match` or `report = no_match`
and numbered `[regex:regexNN]` sections. Composite members are listed under
`rules` in `[composite_rule]`.

#### Part Number Rule Examples

The bundled part number examples contain both a base rule and a composite rule.
`part_number_example_base` checks only the format of non-null strings, looking
for three letters, a dash, and six digits using regex (regular expressions),
such as `ABC-123456`.
`part_number_example_composite` first applies `standard_text_composite`, then
applies that part-number format rule. Use the base rule when only the regex
result matters. Use the composite when missing values, empty strings,
whitespace, suspicious Unicode, and part-number format should be checked as one
ordered rule chain.

#### Part Number Base Rule Directory and Code

The base package is `src/dq/rules/part_number_example_base/`:

| File | Purpose |
| ---- | ------- |
| `__init__.py` | Marks the directory as an importable Python rule package. |
| `rule.ini` | Declares `[rule]`, `[base_rule]`, output behavior, and references to its regex files. |
| `part_number_example.regex` | Defines the extended, case-insensitive part-number regex. |
| `optional_demo_values.py` | Optionally supplies a demo field name and valid and invalid values for `dq_demo`. |

Its important configuration in `rule.ini` is:

```ini
[rule]
description = Part number example: three letters, a dash, and six digits

[base_rule]
report = no_match

[regex:regex01]
regex_file = part_number_example.regex
case_sensitive = false

# ...
```

The directory name supplies the public rule name, so it is not repeated inside
`rule.ini`. This example uses `report = no_match`, which reports values that do
not match any configured regex. Its `[regex:regex01]` section points to
`part_number_example.regex` and controls case sensitivity, full or partial
matching, multiline behavior, and verbose or compact regex syntax. The comments
in `rule.ini` document every setting and its default. See the actual INI file
for complete options and defaults.

Run only the regex rule with:
...
#### Part Number Composite Rule Directory and Code

The composite rule is also defined so the reusable standard text checks run
before the part-number regex. This produces specific findings for null, empty,
whitespace, and suspicious Unicode values instead of treating every problem as
a part-number format failure.

The composite package is `src/dq/rules/part_number_example_composite/`:

| File | Purpose |
| ---- | ------- |
| `__init__.py` | Marks the directory as an importable Python rule package. |
| `rule.ini` | Declares `[rule]`, `[composite_rule]`, automatic field matching, and the ordered rule chain. |
...
The automatic settings make this composite eligible for text/string fields
whose names contain the separate components `part` and `number`. The ordered
chain reports a preliminary standard-text failure before considering the regex.
See the actual INI file for complete options and defaults.
...
If a rule defines `optional_demo_values.py`, generate and submit `dq_demo` as
described above. First confirm that its suggested field was created. For the
part-number example, run:

```sh
bin/dq --collection dq_demo --list_fields
```

Look for `part_number_s` in the field list. The base rule can then test only the
part-number regex. Adding `--skip_null_values true` keeps the test focused on
non-null strings:

```sh
bin/dq --collection dq_demo --rule part_number_example_base \
  --include_field part_number_s --skip_null_values true
```

The default CSV action writes
`reports/part_number_s_part_number_example_base.csv`. Its `reason` column should
identify regex failures, and its `value` column should show the generated value
that failed. Remove `--skip_null_values true` when also verifying that nulls are
reported safely.

Next, run the composite rule:

```sh
bin/dq --collection dq_demo --rule part_number_example_composite \
  --include_field part_number_s
```

This writes `reports/part_number_s_part_number_example_composite.csv`. Look for
standard text failures such as null, empty, or whitespace values, along with
part-number regex failures. Because a composite runs in order, the `reason`
column contains the first failed base rule for each stored value.
...
text or Regex rule. Base and composite rules can declare automatic field types
and field-name patterns in the common `[rule]` section of their own `rule.ini`
files. The current automatic format checks are composites so their standard
missing, empty, whitespace, and Unicode checks run before the format regex.

DQ converts field names to lowercase components, splitting at punctuation,
underscores, and camel-case boundaries. Each line in
`automatic_field_name_patterns` is an alternative; `+` requires multiple whole
components:
...
| `part_number_example_composite` | both `part` and `number` | `part_number_s`, `legacyPartNumber` |
...
If one field matches more than one specialized rule, checkup uses the first rule
name alphabetically. Quick and full checkup therefore make the same selection.
The quick-checkup Fields table identifies the selected rule and every other
specialized rule that matched. The type-only `standard_text_composite` fallback
does not participate when a specialized rule matches.

### Synthetic Test Value Generation

As previously mentioned, DQ can generate both valid and invalid synthetic data
into a `dq_demo` collection/index. Ideally, a new custom rule also contributes
applicable test data to `dq_demo`. To contribute its own field, a base rule must
include `optional_demo_values.py` in its rule directory. That Python file must
define these functions:

| Function | Purpose |
| -------- | ------- |
| `suggested_field_name()` | Returns the field name to add to `dq_demo`. For the MVP, it must end in `_t` for analyzed text or `_s` for an exact string/keyword. |
| `generate_valid_value(index, rng)` | Returns a valid value for the one-based document number `index`. |
| `generate_invalid_value(index, rng)` | Returns a non-null value that should fail the rule. It can use `rng` to select from several understandable failure cases. |

The scripts in `generate_test_collection/` discover these files and create the
same logical records for Solr, Elasticsearch, and OpenSearch. The engines'
dynamic schemas assign the suggested field's type, so it does not need a
separate central schema entry.

Each field starts with a valid synthetic value. The generator then independently
selects `--incorrect_percent` of that field's records and distributes them
across several defect types. Selection is random by default; `--seed` makes a
run repeatable. The main generator creates one `random.Random` object from the
run's seed and passes that same object to every call. Rules use `rng.choice()`
when they offer several invalid formats. They do not create or reseed private
random generators, so the same seed and options reproduce the same output.
...
When generating ordinary text fields, Unicode corruption is kept rare:
approximately 0.1% of all records at the standard 20% setting. The remaining
selected records are divided among null, blank, and the readable whitespace
examples. Small generated sets contain at least one Unicode example so the rule
can still be demonstrated.

When generating text fields, DQ creates null values, empty strings,
whitespace-only strings, leading or trailing whitespace, and malformed values.
The three nonblank whitespace examples are ` leading whitespace example`,
`trailing whitespace example `, and ` leading and trailing whitespace example `.
Malformed general text randomly selects from six combinations of suspicious
Unicode characters. Each combination spans at least three code-point buckets,
...
fields instead randomly select from several violations of their expected format.

| Generated field(s) | Valid example | Invalid-value variety | Rule exercised |
| ------------------ | ------------- | --------------------- | -------------- |
| Names, street, city, state, postal code, country, notes | `DemoName7`, `7 Example Street`, `Example City` | Null, blank, surrounding whitespace, and six Unicode combinations spanning at least three code-point buckets | `standard_text_composite` |
...
| `part_number_s` | `ABC-000007` | Missing/wrong separator, letters in the prefix, wrong digit count, or extra suffix | `part_number_example_composite` when selected explicitly |
...
are in `tests/test_demo_generator.py`. Instructions and commands for creating
`dq_demo` are in the **Generate a Test Collection** section above.

### Quick Checkup Presence Counts

Quick checkup uses search-engine field-presence queries and counts each document
once per field. These counts cover the entire collection/index even when
`--rows` limits the full-checkup estimate or a later document scan. Native date
and vector fields receive presence counts only in quick checkup; full checkup
runs `missing_fields_base` row by row.
