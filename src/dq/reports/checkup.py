...
from dq.solr import list_fields, collection_document_count, field_document_count
from dq.processors import ReportError
from dq.processors._checkup.processor import plan, overrides, scan, ordered_checks
...
def write_report(target, output_path, *, include=(), exclude=(), connection=None,
    ...
    progress.update('loading Solr schema and field inventory', force=True)
    ...
    rules = overrides(configuration_path)
    plans = dict((f['name'], plan(f, rules)) for f in selected)
    ...
    for index, field in enumerate(selected, 1):
        name = field['name']
        if 'missing_fields' in plans[name]:
            progress.update('field {0:,}/{1:,} {2}; test missing_fields (indexed presence)'.format(index, len(selected), name), force=True)
    ...
    value_fields = [field for field in selected if set(plans[field['name']]) - {'missing_fields'}]
    ...
    for index, field in enumerate(selected, 1):
        ...
        if not plans[name]:
            selections.append((_code(name), 'none', 'disabled by explicit [checkup] override'))
        ...
        lines += _table(('Check', 'Findings'), [(check, '{0:,}'.format(result['counts'][check])) for check in ordered_checks(plans[name]) if check != 'missing_fields'])
    ...
    lines += ['Checks inferred from names are suggestions; review Check Selection. A finding is not necessarily an error.',
              'Missing counts use Solr field existence, not direct null inspection of stored records.',
...
def _full_workload(selected, plans, total, row_limit=-1):
    fields = [field for field in selected
              if set(plans[field['name']]) - {'missing_fields'}]
    checks = {}
    for field in fields:
        for check in set(plans[field['name']]) - {'missing_fields'}:
            checks[check] = checks.get(check, 0) + 1
    lines = ['## Full Report Workload Estimate', '',
             'Scope: all fields selected in this report, using the same rows limit and checkup overrides.', '',
             '- Collection documents: {0:,}'.format(total),
             '- Fields with presence checks: {0:,}'.format(sum('missing_fields' in plans[f['name']] for f in selected)), '']
    if not fields:
        if any(plans[field['name']] for field in selected):
            ...
            lines += ['All checks are disabled by the checkup overrides for this selection. '
                      'No field checks or stored-value scan will run.', '']
        return lines
...
def _write_lite(target, output_path, selected, plans, total, missing, options,
    ...
    for field in selected:
        name = field['name']
        suggestions = ordered_checks(set(plans[name]) - {'missing_fields'})
        rows.append((_code(name), _code(str(field.get('type', 'unknown'))),
                     '{0:,}'.format(total - missing[name]) if name in missing else 'not checked',
                     '{0:,}'.format(missing[name]) if name in missing else 'not checked',
                     ', '.join(suggestions) or ('Presence check only' if 'missing_fields' in plans[name] else 'Disabled')))
    lines = ['# Automatic Checkup — Quick', '', 'Report: ' + _code(report_name), '', '- [Summary](#summary)',
             ...
             '- Exclude patterns: ' + _code(', '.join(exclude if include or exclude else DEFAULT_EXCLUDED_FIELDS) or 'none'),
             '',
             'Document counts come from Solr field-presence queries, counting each document once per field.',
    ...
    lines += ['', '## Fields', ''] + _table(('Field', 'Solr type', 'Docs w/Value', 'Docs w/o Value', 'Additional checks in full report'), rows)
    lines += ['', 'Run `bin/dq --report full_checkup` to perform these additional checks.', '',
              ...
              'Suggested checks depend on schema types and field names: standard_text for text/string fields; '
              'email, us_phone, and ssn for matching name components. Native date fields get presence checks only. '
...
def _followup_examples(target, selected, plans, missing, configuration, configuration_explicit, row_limit):
    ...
    for field in selected:
        ...
        checks = [check for check in ordered_checks(plans[name])
                  if check not in ('missing_fields', 'empty_values')]
        specialized = [check for check in checks if check in ('email', 'us_phone', 'ssn')]
        rule = (specialized or checks or
                (['missing_fields'] if missing.get(name, 0) else [None]))[0]
