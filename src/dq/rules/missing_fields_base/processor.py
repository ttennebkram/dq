...
from dq.search import list_fields
...
def prepare_csv(target, *, include=(), exclude=(), connection=None, row_limit=-1,
    ...
    if not fields:
        raise SolrError('missing_fields_base requires stored fields; adjust include/exclude field patterns')
    def rows():
        ...
        for identifier, name, present in values:
            if present is False:
                yield Finding(name, (identifier, 'missing_fields_base: missing or null', ''))
            elif present is not True:
                raise SolrError('search engine did not return a boolean field-presence value')
