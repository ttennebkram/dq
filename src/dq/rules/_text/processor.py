...
from dq.rules.code_points_base.processor import reasons
...
def value_reasons(value, skip_null_values=False):
    ...
    return ['code_points_base: ' + unicode_reasons[0]] if unicode_reasons else []
...
def fields(target, include=(), exclude=(), connection=None):
    ...
    if not selected:
        raise ReportError('standard_text_composite: no text/string fields selected')
    return selected
