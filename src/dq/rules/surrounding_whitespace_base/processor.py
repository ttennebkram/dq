"""Export strings with leading or trailing whitespace."""
...
def fields(target, include=(), exclude=(), connection=None):
    ...
    if not selected:
        raise ReportError('surrounding_whitespace_base: no text/string fields selected')
    return selected
        ...
        for identifier, field, value in values:
            if isinstance(value, str) and value != value.strip():
                yield Finding(field, (identifier,
                                      'surrounding_whitespace_base: leading or trailing whitespace',
                                      value))
